...
#### data
path=${path:-"data_submit/coco-v1-2s/usps-loss-below=0.85-100-attack-gpt-4.json"}
...
task_config=${task_config:-"attack-synonym.yaml"}
...
run_exp() {
    python src/run_dpo_diff.py \
        --save $id --gpu $gpu \
        --path $path --prompt_id $1 --version $version \
        --num_seeds=$num_seeds --num_exps=$num_exps \
